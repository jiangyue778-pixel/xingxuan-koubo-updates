...
- 私有源码仓库：由维护者管理，不在此公开。
- Windows 程序：首次优先双击 release 中的 `xingxuan-koubo-windows-installer-*.exe`，它会自动解压并准备专用运行环境、转写模型、桌面快捷方式和开机启动；之后自动读取 `update.json`，下载并校验新版更新包。
...
请先运行 Windows 单文件安装程序。安装完成后本地服务会静默运行，不需要另外安装 Python、手动解压或保持命令窗口开启。
